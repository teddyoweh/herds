...
# Produces dist/*.whl with the full UI baked in (pip install → darwin host → UI).
set -euo pipefail
cd "$(dirname "$0")/.."

echo "→ building dashboard (static export)…"
( cd web && NEXT_PUBLIC_DARWIN_API="" pnpm build )

echo "→ bundling dashboard into package…"
rm -rf src/darwin/web_dist
cp -R web/out src/darwin/web_dist
